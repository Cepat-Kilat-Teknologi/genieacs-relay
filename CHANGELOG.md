...
| Total downtime | **6:52** |
| Device re-inform to ACS | Yes (reboot preserves `ConnectionRequestUsername`/`Password`) |
...
| Device web UI | Yes 200 OK at `http://<ip>/` post-recovery |
...
- **`POST /api/v1/genieacs/diag/ping/{ip}` (M1)**: TR-069
  ...
  trigger flag, which TR-069 sectionA.4.1 mandates be the LAST entry so the
...
- [x] `API_REFERENCE.md` section 14 (reboot slow-boot note) + section 17.1 (factory-reset real-device verification + upstream blocker)
